...
class SemgrepModel extends Model
{
    ...
    public static function getDetailCount()
    {
        ...
        $countList = [
            ['name' => '风险总数(个)',
                'num' => Db::table('semgrep')->where(['project_id' => 1])->count(),
                'lists' => [
            ...
            ['name' => '受影响仓库(个)', 'num' => Db::table('semgrep')->where(['project_id' => 1])->group('git_addr')->count(),
                'lists' => [
                    '总数 ' => Db::table('semgrep')->where(['project_id' => 1])->group('git_addr')->count(),
                    '7天新增 ' => Db::table('semgrep')->whereTime('create_time', '>=', date('Y-m-d', time() - (7 * 86400)))->where(['project_id' => 1])->group('git_addr')->count()
                ]
            ],
